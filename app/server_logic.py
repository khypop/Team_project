...
ai_engine = ctypes.CDLL("./fsrcnn_engine.dll")

def process_frame_with_dll(frame):
    data_ptr = frame.ctypes.data_as(ctypes.POINTER(ctypes.c_ubyte))
    ai_engine.upscale_frame(data_ptr, frame.shape[1], frame.shape[0])
    return frame
...
class WebRTCServer:
    ...
    async def run(self):
        ...
        self.conn = await websockets.connect("wss://worker-collectors-incomplete-vocational.trycloudflare.com") #시그널링 서버로 연결
